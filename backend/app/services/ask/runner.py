    ...
    if isinstance(obj, list):
        for it in obj:
            if isinstance(it, dict):
                return it
        raise ValueError("JSON list has no object")
    ...
    if plan is None and use_llm:
        if not LLM_ENABLED:
            ...
            try:
                ...
                metrics, dimensions, filters, sort, limit, notes.

                Rules:
                - Use ONLY these metric keys: {", ".join(allowed_metrics)}
                - Use ONLY these dimension keys: {", ".join(allowed_dims)}
                - Allowed filter ops: =, !=, >, >=, <, <=, in, between, ilike
                ...
                """.strip()
                        ...
                        res2 = await call_gemini_with_budget(g, repair_prompt)
                        llm_ms = (llm_ms or 0) + res2.ms
                        llm_attempts = (llm_attempts or 0) + res2.attempts
                        llm_status = res2.status

                        if res2.status == "ok":
                            plan = _extract_json(res2.text)
                            used_llm = True
                        else:
                            plan = None
                            used_llm = False
                            llm_error = f"{res2.status}: {res2.error}"
                else:
                    plan = None
                    used_llm = False
                ...
                warnings_outer.append(f"LLM failed: {llm_error}")
                print("LLM_RAW_FIRST_300=", (res.text or "")[:300])
